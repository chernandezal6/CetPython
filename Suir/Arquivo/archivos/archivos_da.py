...
def validar_encabezado(linea, stack_msg):
	...
	if not funciones.is_rnc_o_cedula(rnc) or rnc.startswith(" ") :
...
def validar_detalle(linea, stack_msg):
	...
	if not funciones.is_documento_valido('C', numero_documento, 25):
	...
	if tipo_reclamacion == "P":
		...
		if (salario != '0000000000000000'):
			prevalidaciones_msg="En la linea %s error en el salario ss (%s)" % (iLineas, salario)
			return False			

		if (aporte_voluntario != '0000000000000000'):
			prevalidaciones_msg="En la linea %s error en el aporte voluntario (%s)" % (iLineas, aporte_voluntario)
			return False
...
def insertar_detalle_bind(registros, nro_linea, linea, id_envio,id_error, status, usuario_carga, stack_msg_str):
	...
	try:	
		...
		registros.append(( id_envio, nro_linea,'C',numero_documento, periodo_aplicacion,id_reclamacion, salario, aporte_voluntario, nro_referencia, tipo_reclamacion, motivo_devolucion,id_error, status, usuario_carga,stack_msg_str)) 
