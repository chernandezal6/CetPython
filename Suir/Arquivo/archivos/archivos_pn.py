...
def validar_detalle(linea, stack_msg):
	...
	if not funciones.is_tipo_novedad(tipo_novedad,"PN"):
		stack_msg.append("En la linea %s error en el Tipo de Novedad (%s)" % (iLineas, tipo_novedad))
		return False	
...
def insertar_detalle_bind(registros, nro_linea, linea, id_envio,id_error, status, usuario_carga,stack_msg_str):
	try:
		...
		registros.append((id_envio, nro_linea,tipo_novedad, IdARS_encabezado, nro_formulario, id_pensionado, id_nss, nombres, primer_apellido, segundo_apellido, tipo_documento, no_documento,id_error, status, fecha_nacimiento, id_nacionalidad, sexo,
...
def cargar_movimiento(id_envio, oCurs, stack_msg):	
	v_result = "0"
	try:
		oCurs.callproc("sre_procesar_python_pkg.Procesar_Pn",[id_envio, v_result])
