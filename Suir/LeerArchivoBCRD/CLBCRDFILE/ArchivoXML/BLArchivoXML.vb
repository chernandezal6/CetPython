...
#Region "Metodos publicos de la clases"
    ...
    Public Sub EnviarCorreo()
        ...
        If query.Count > 0 Then
            ...
            For Each sfile As FileInfo In query
                ...
                If sfile.Extension.Equals(".xml") Then
                    ...
                    EnviarConfirmacion(Body(), "Archivo de Liquidación de fondos" + " " + "del" + " " + entidadnombre, TipoArchivo.Procesados)
                ...
                ElseIf sfile.Extension.Equals(".txt") Or sfile.Extension.Equals(".TXT") Then
                    ...
                    'Enviar Confirmacion
                    EnviarConfirmacion(BodyConcentracion(), "Archivo de Concentración" & " " & "del" & " " & entidadOrigen, TipoArchivo.Procesados, rutaachivoprocesado)
    ...
    Public Sub ProcesarArchivos()
        ...
        If EnumFiles.Count > 0 Then
            ...
            ''Ver si hay archivo Zip en el directorio de archivo Zip.
            Dim mydirzip As New DirectoryInfo(Me.XMLZip)

            'Extraer todos los archivos Zip.
            Dim myfilezip() As FileInfo = mydirzip.GetFiles("*.Zip", SearchOption.TopDirectoryOnly)

            'Buscar los archivos Zip del dia
            Dim queryzip = From fizip In myfilezip _
                        Where fizip.LastWriteTime.Date = DateTime.Now.Date _
                        Select fizip

            'si no existe entonce no se ha procesado ningun archivo por primera vez.
            If queryzip.Count = 0 Then

                Dim mydir As New DirectoryInfo(Me.ArchivosProcesados)

                'Obtengo una lista con todos los archivos que han sido procesados
                Dim myfiles() As FileInfo = mydir.GetFiles("*.xml", SearchOption.TopDirectoryOnly)
                ...
                If query.Count > 0 Then
                    ...
                    Dim ArchivoZip As String = "ArchivosXML" + String.Format("{0:yyyyMMdd}", DateTime.Now.Date) + ".Zip"

                    Dim rutaachivozipprocesado As String = Me.ArchivosProcesados & "\" & ArchivoZip

                    'Comprimir Archivo
                    FolderZipper.ZipUtil.ZipFiles(ArchivosProcesados, ArchivoZip)

                    Dim rutaachivoZip As String = Me.XMLZip + "\" + ArchivoZip

                    MoverArchivoEnc(rutaachivozipprocesado, rutaachivoZip)

                    EnviarConfirmacion("", "Archivos para ser procesados en el SRIF" + " " + "del dia" + " " + String.Format("{0:yyyyMMdd}", DateTime.Now.Date), TipoArchivo.Zip, rutaachivoZip)
                End If
            Else
                Dim myfileerror As New DirectoryInfo(Me.ArchivosConError)

                Dim myfileserror() As FileInfo = myfileerror.GetFiles("*.xml", SearchOption.TopDirectoryOnly)

                'buscar los archivos que dieron error en dia.
                Dim queryerror = From fierror In myfileserror _
                                 Where fierror.LastWriteTime.Date = DateTime.Now.Date _
                                 Select fierror

                'obtener la cantidad de registro con error.
                Dim v_registro_error As Int32 = queryerror.Count

                'buscar todos los archivos procesados.
                Dim dirpr As New DirectoryInfo(Me.ArchivosProcesados)

                'todos los archivos que son de Liquidacion
                Dim myfilespr() As FileInfo = dirpr.GetFiles("*.xml", SearchOption.TopDirectoryOnly)

                'todos los archvios del dia organizado desendentemente.
                Dim querypr = From fi In myfilespr _
                            Order By fi.LastWriteTime Descending _
                            Where fi.LastWriteTime.Date = DateTime.Now.Date _
                            Select fi

                'Obtener solo los registros que dieron error que ya estan procesados correctamente.
                Dim Enumpr As IEnumerable(Of FileInfo) = querypr.Take(v_registro_error)

                'Hacer Metodo en el objecto FolderZipper.ZipUtil que acepte un IEnumerable con los archivos que ahi que reprocesar.

                Dim ArchivoRepZip As String = "ArchivosRepXML" + String.Format("{0:yyyyMMdd}", DateTime.Now.Date) + ".Zip"

                Dim rutaachivozipprocesado As String = Me.ArchivosProcesados & "\" & ArchivoRepZip

                'Comprimir Archivo
                'FolderZipper.ZipUtil.ReprosarZip(Enumpr, ArchivosProcesados, ArchivoRepZip)

                Dim rutaachivoZip As String = Me.XMLZip + "\" + ArchivoRepZip
...
#Region "Funciones Internas para la validar el archivo xml"
    ...
    Protected Function ProcesarArcLiquidacion() As String
        ...
        Catch oex As OracleException
            Set_Estatus_Error("ERAP", v_sec_log, 59, oex.Message)
            MoverArchivoError("Ha ocurrido el siguiente error Agregando este archvio : " + "" + oex.Message, fileinfo.FullName, "Archivo XML")
        End Try
        Return Me.resultado
    End Function
    Protected Friend Sub EnviarConfirmacion(ByVal body As String, ByVal Asunto As String, ByVal tipoArchivo As TipoArchivo, Optional ByVal file1 As String = Nothing)

        Select Case tipoArchivo

            Case BLArchivoXML.TipoArchivo.Procesados
                'Archivos de Concentracion
                If (Not String.IsNullOrEmpty(file1)) Then
                    ...
                    Try
                        ...
                        message.Attachments.Add(New Attachment(file1))
                ...
                Else
                    ...
                    Dim direccionfrom As New MailAddress("info@mail.tss2.gov.do")
                    Dim mymessage As New MailMessage
                    Dim mysmtp As New SmtpClient
                    Dim correosok As String = String.Empty
                    Dim correoserror As String = String.Empty
                    Dim direciones As String = String.Empty

                    Try
                        getMailAddress(correosok, correoserror)
                        direciones = correosok & "," & correoserror
                        mymessage.From = direccionfrom
                        mymessage.To.Add(correosok)
                        mymessage.Subject = Asunto
                        mymessage.IsBodyHtml = True
                        mymessage.Body = body
                        mymessage.Priority = MailPriority.Normal
                        mymessage.BodyEncoding = System.Text.Encoding.Default
                        mysmtp.Host = "172.16.5.8"
            ...
            Case BLArchivoXML.TipoArchivo.ConError
                ...
                Catch sex As SmtpException
                    Set_Estatus_Error("ERAP", v_sec_log, nombrearchivo, 59, sex.Message)
                End Try
            Case BLArchivoXML.TipoArchivo.NoExiste
                ...
                Catch sex As SmtpException
                    Set_Estatus_Error("ERAP", v_sec_log, nombrearchivo, 59, sex.Message)
                End Try
            Case BLArchivoXML.TipoArchivo.Aplicacion
                Dim direccionfrom As New MailAddress("info@mail.tss2.gov.do")
                Dim mymessage As New MailMessage
                Dim mysmtp As New SmtpClient
                Dim correosok As String = String.Empty
                ...
                Catch sex As SmtpException
                    Set_Estatus_Error("ERAP", v_sec_log, nombrearchivo, 59, sex.Message)
                End Try
            Case BLArchivoXML.TipoArchivo.Zip
    ...
    ''' <remarks></remarks>
    Public Sub getMailAddress(ByRef listok As String, ByRef listerror As String)
    ...
    Private Sub Test()
        Dim mydir As New DirectoryInfo(Me.ArchivosProcesados)
        Dim mydiren As New DirectoryInfo(Me.ArchivoRecibidosEnc)

        'Obtengo una lista con todos los archivos que han sido procesados
        Dim myfiles() As FileInfo = mydir.GetFiles("*.xml", SearchOption.TopDirectoryOnly)

        Dim myfilesen() As FileInfo = mydiren.GetFiles("*.xml", SearchOption.TopDirectoryOnly)

        'de la lista selecciono solo los archivos del dia
        Dim query = From fi In myfiles _
                    Join fe In myfilesen On fi.Name Equals fe.Name _
                    Order By fi.LastWriteTime Descending _
                    Where fi.LastWriteTime.Date = DateTime.Now.Date _
                    Select fi

        Dim v_registro As Integer = query.Count
        For Each archivo As FileInfo In query
            Console.WriteLine("Archivo : {0}", archivo.Name)
        Next
        Console.ReadLine()
    End Sub
    ...
    Protected Friend Sub DecryptFile(ByVal FileIn As String, ByVal Fileout As String, ByVal key As String, ByVal IV As String, ByVal asunto As String, ByRef resultado As String)
        ...
        'processstar.FileName = Me.javadir + "java.exe" -- YO NO TENGO ESE JAVADIR
        ...
        Try
            ...
            If String.IsNullOrEmpty(outStr) Then
                ...
                MoverArchivoError("Ocurrio el siguiente error descriptando el archivo" + "" + outStr, fileinfo.FullName, asunto)
            End If

        Catch pex As Win32Exception
            If pex.NativeErrorCode = ERROR_FILE_NOT_FOUND Then
                Set_Estatus_Error("ERAP", v_sec_log, 59, pex.NativeErrorCode)
           ...
           & "<table style=" & """" & "width:100%;" & """>" _
             & "<tr>" _
                 & "<td class=" & """" & "style3" & """>" _
                     & "&nbsp;</td>" _
                 & "<td class=" & """" & "style5" & """ >" _
                        & "Reporte de Liquidación de Fondos</td>" _
                 & "<td>" _
                     & "&nbsp;</td>" _
             & "</tr>" _
             & "</table>" _
    ...
    Protected Function BodyConcentracion() As String
        ...
        Dim EntidadNombre As String = String.Empty
        Dim EnumDetalle As IEnumerable(Of String) = File.ReadAllLines("C:\Users\hminaya.TSS2\Desktop\Arc").Where(Function(p As String) _
                                                                                                        p.StartsWith("D"))
  ...
  & "<body>" _
    ...
    Protected Sub getDesProceso(ByVal TRN As String, ByRef proceso As String, ByRef result As String)
        ...
        End Try
    End Sub
    Protected Sub getEntidadNombrePorBic(ByVal bic As String, ByRef entidadnombre As String, ByRef result As String)
...
#Region "Enumeraciones"
    Protected Friend Enum TipoArchivo
        Procesados
        ConError
        NoExiste
        Aplicacion
