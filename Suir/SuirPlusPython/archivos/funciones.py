...
def is_fecha(campo):
	...
	if  ((meses > "0") and (meses <="12") and (year >= "1901") ):
		return True
	else :
		return False	
...
#funcion  para calcular el limite de fecha calendario de ingreso y salida trabajadores en la novedad
def is_fecha_limite_nov(campo,tipo_archivo):
	if tipo_archivo in ("NV", "AM", "AR"):
		if not len(campo) == 8:
			return False
		...
		calcdate = fecha_a_calcular - datetime.now().date() 
		# Imprimo el resultado final
		if int(calcdate.days) > 30:
			return False
		else:
			return True	
...
def is_nombre_propio(tipo,campo, longitud):
	if tipo == "P":
