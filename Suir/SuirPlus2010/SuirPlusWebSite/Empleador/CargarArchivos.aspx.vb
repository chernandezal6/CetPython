...
Partial Class Empleador_CargarArchivos
    Inherits BasePage
    ...
    Protected Sub lnkBtnCargarArchivo_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles lnkBtnCargarArchivo.Click
        ...
        Dim ArchivoPosteadopy As ManejoArchivoPython
        ...
        Try
            ...
            End If
            '**
            'buscar parametro procesar_py en tipoArchivo

            If (Utilitarios.Utils.procesarPY(Me.ddlProceso.SelectedValue.ToString)) Then
                SuirPlus.Exepciones.Log.LogToDB("Inicio carga de archivos via proceso python...")
                If (getTipoArchivo(Me.ddlProceso.SelectedValue.ToString) = Archivo.SuirArchivoType.NovedadesPensionados) Or (getTipoArchivo(Me.ddlProceso.SelectedValue.ToString) = Archivo.SuirArchivoType.TraspasoPensionados) Then
                    ArchivoPosteadopy = New ManejoArchivoPython(fileName, data, getTipoArchivo(Me.ddlProceso.SelectedValue.ToString), Me.getUsername, Me.getARSUsuario, Me.GetIPAddress())
                Else
                    ArchivoPosteadopy = New ManejoArchivoPython(fileName, data, getTipoArchivo(Me.ddlProceso.SelectedValue.ToString), Me.getUsername, getNSS, getRNCEmpleador(), UsrIDEntidadRecaudadora, GetIPAddress())
                End If

                Me.lblNombreArchivo.Text = ArchivoPosteadopy.NombreAchivo
                Me.lblNumeroArchivo.Text = ArchivoPosteadopy.Numero
                SuirPlus.Exepciones.Log.LogToDB("Fin carga de archivos via proceso python..." + ArchivoPosteadopy.NombreAchivo + " - " + ArchivoPosteadopy.Numero.ToString())
            Else
                SuirPlus.Exepciones.Log.LogToDB("Inicio carga de archivos via proceso .Net...")
                If (getTipoArchivo(Me.ddlProceso.SelectedValue.ToString) = Archivo.SuirArchivoType.NovedadesPensionados) Or (getTipoArchivo(Me.ddlProceso.SelectedValue.ToString) = Archivo.SuirArchivoType.TraspasoPensionados) Then
                ...
                SuirPlus.Exepciones.Log.LogToDB("Fin carga de archivos via proceso .Net..." + ArchivoPosteado.NombreAchivo + "-" + ArchivoPosteado.Numero.ToString())
            End If
