...
Partial Class Subsidios_consSubsidiosSFS
    Inherits BasePage
    ...
    Protected Sub CargarSubsidiosSFS()
        Try
            ...
            dtSubsidiosSFS = SubsidiosSFS.Consultas.getSubsidiosSFS(UsrRNC, txtCedula.Text, ddlEstatusSubsidios.SelectedValue, ddlTipoSubsidio.SelectedValue, Me.txtFechaDesde.Text, Me.txtFechaHasta.Text, Me.pageNum, Me.PageSize)

            If IsNothing(dtSubsidiosSFS) Then
    ...
    Protected Sub gvSubsidiosSFS_RowCommand(ByVal sender As Object, ByVal e As System.Web.UI.WebControls.GridViewCommandEventArgs) Handles gvSubsidiosSFS.RowCommand
        ...
        Try
            If e.CommandName = "VerDetalle" Then
                ...
                'If (Left(tipo, 10).ToLower = "enfermedad") Then
                '    Response.Redirect("DetSubsidiosSFS.aspx?IdSolicitud=" & idSol & "&Tipo=EF")
                'Else
                '    Response.Redirect("DetSubsidiosSFS.aspx?IdSolicitud=" & idSol & "&Tipo=" & tipo)
                'End If
            ...
            ElseIf e.CommandName = "Rec" Then
                Dim idRegistro As String = Split(e.CommandArgument, "|")(2)
                If tipo = "M" Then
                    Response.Redirect("Sol.aspx?m=r" & "&n=" & idRegistro & "&p=m")
                ElseIf tipo = "L" Then
                    Response.Redirect("Sol.aspx?m=r" & "&n=" & idRegistro & "&p=l")
                ElseIf tipo = "E" Then
                    Response.Redirect("EnfermedadComun.aspx?m=r" & "&n=" & idRegistro & "&p=e")
                End If
            End If
        Catch ex As Exception
            Me.lblMensaje.Visible = True
    ...
    Protected Sub gvSubsidiosSFS_RowDataBound(ByVal sender As Object, ByVal e As System.Web.UI.WebControls.GridViewRowEventArgs) Handles gvSubsidiosSFS.RowDataBound
