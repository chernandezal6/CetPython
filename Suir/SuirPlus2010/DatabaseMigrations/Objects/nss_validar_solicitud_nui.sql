CREATE OR REPLACE PROCEDURE SUIRPLUS.NSS_VALIDAR_SOLICITUD_NUI
(
...
BEGIN
  ...
  LOOP
    BEGIN
      ...
      IF v_conteo >= 1 THEN
        ...
        IF p_resultado <> 'OK' THEN
          ...
          suirplus.detalle_bitacora(v_id_bitacora,
                                    sysdate,
                                    SUBSTR('Solicitud ASIGNACION NSS A MENOR CON NUI, ID_SOLICITUD #'||to_char(P_ID_SOLICITUD)||', ID_REGISTRO #'||to_char(C_SOL.ID_REGISTRO)||': ignorada ('||to_char(sysdate,'dd/mm/yyyy hh:mi:ss am')||').'||chr(13)||chr(10)||
                                    'MOTIVO: '||p_resultado,1,500));
        ELSE
          -- Grabar primer seguimiento en detalle de bitacora
          suirplus.detalle_bitacora(v_id_bitacora,
                                    sysdate,
                                    SUBSTR('Solicitud ASIGNACION NSS A MENOR CON NUI, ID_SOLICITUD #'||to_char(P_ID_SOLICITUD)||', ID_REGISTRO #'||to_char(C_SOL.ID_REGISTRO)||': rechazada ('||to_char(sysdate,'dd/mm/yyyy hh:mi:ss am')||').'||chr(13)||chr(10)||
      ...
      v_jce_respuesta := suirplus.NSS_WEBSERVICE_JCE_NUI(C_SOL.NO_DOCUMENTO_SOL);
      ...
      BEGIN
        v_sre_ciudadano.NUMERO_EVENTO  := SUIRPLUS.NSS_PARSEAR_NUI(v_jce_respuesta, 'numeroevento');
      EXCEPTION WHEN OTHERS THEN
      ...
      v_sre_ciudadano.ID_NACIONALIDAD  := '1'; --Fijo, ya que no viene desde la JCE
      ...
      IF v_conteo > 0 THEN --Existe en ciudadanos
        ...
        IF v_conteo > 0 THEN
          ...
          ELSE
            ...
            suirplus.detalle_bitacora(v_id_bitacora,
                                      sysdate,
                                      SUBSTR('Solicitud ASIGNACION NSS A MENOR CON NUI, ID_SOLICITUD #'||to_char(P_ID_SOLICITUD)||', ID_REGISTRO #'||to_char(C_SOL.ID_REGISTRO)||': procesada ('||to_char(sysdate,'dd/mm/yyyy hh:mi:ss am')||').',1,500));
          END IF;
          ...
          IF v_id_evaluacion IS NULL THEN
            ...
            BEGIN
              ...
              ELSE
                ...
                suirplus.detalle_bitacora(v_id_bitacora,
                                          sysdate,
                                          SUBSTR('Solicitud ASIGNACION NSS A MENOR CON NUI, ID_SOLICITUD #'||to_char(P_ID_SOLICITUD)||', ID_REGISTRO #'||to_char(C_SOL.ID_REGISTRO)||': puesta en evaluacion visual ('||to_char(sysdate,'dd/mm/yyyy hh:mi:ss am')||').',1,500));
              END IF;
            END;
          ELSE
            -- Insertamos en el detalle de Evaluacion Visual
      ...
      ELSIF v_conteo = 1 THEN
        ...
        ELSE
          ...
          suirplus.detalle_bitacora(v_id_bitacora,
                                    sysdate,
                                    SUBSTR('Solicitud ASIGNACION NSS A MENOR CON NUI, ID_SOLICITUD #'||to_char(P_ID_SOLICITUD)||', ID_REGISTRO #'||to_char(C_SOL.ID_REGISTRO)||': actualizo en ciudadano ('||to_char(sysdate,'dd/mm/yyyy hh:mi:ss am')||').',1,500));
        END IF;
      ...
      ELSE
        ...
        IF v_conteo > 0 THEN
          FOR R IN (SELECT * 
            ...
            SET tipo_documento = 'P',
                no_documento   = 'P'||TRIM(no_documento)
        ...
        ELSE
          ...
          IF v_conteo > 0 THEN
            FOR R in (SELECT ID_NSS
                        FROM suirplus.sre_ciudadanos_t
                       WHERE no_documento = v_sre_ciudadano.NO_DOCUMENTO) LOOP
              --Rechazamos la solicitud
              SUIRPLUS.NSS_ACTUALIZAR_SOLICITUD(C_SOL.ID_REGISTRO, 6, 'NSS001', R.ID_NSS, p_ult_usuario_act, p_resultado); --Este Ciudadano ya posee Cedula
      ...
      ELSE
        ...
        suirplus.detalle_bitacora(v_id_bitacora,
                                  sysdate,
                                  SUBSTR('Solicitud ASIGNACION NSS A MENOR CON NUI, ID_SOLICITUD #'||to_char(P_ID_SOLICITUD)||', ID_REGISTRO #'||to_char(C_SOL.ID_REGISTRO)||': creo el ciudadano ('||to_char(sysdate,'dd/mm/yyyy hh:mi:ss am')||').',1,500));
      END IF;
    EXCEPTION WHEN OTHERS THEN
      ...
      suirplus.detalle_bitacora(v_id_bitacora,
                                sysdate,
                                SUBSTR('Solicitud ASIGNACION NSS A MENOR CON NUI, ID_SOLICITUD #'||to_char(P_ID_SOLICITUD)||', ID_REGISTRO #'||to_char(C_SOL.ID_REGISTRO)||': ignorada por error: '||p_resultado,1,500));
    END;
  END LOOP;
