...
namespace SuirPlus.Utilitarios
{
	...
	public class TiffManager : IDisposable
	{
        ...
        public ArrayList SplitTiffImageStream(byte[] dbFile, EncoderValue format)
        {
            MemoryStream ms = new MemoryStream(dbFile);
            Image newImage = Image.FromStream(ms);

            ArrayList splitedFiles = new ArrayList();
            try
            {
                Guid objGuid = newImage.FrameDimensionsList[0];
                FrameDimension objDimension = new FrameDimension(objGuid);

                //Saves every frame as a separate file.
                Encoder enc = Encoder.Compression;
                int pageNo = newImage.GetFrameCount(objDimension);
                int curFrame = 0;
                for (int i = 0; i < pageNo; i++)
                {
                    newImage.SelectActiveFrame(objDimension, curFrame);
                    EncoderParameters ep = new EncoderParameters(1);
                    ep.Param[0] = new EncoderParameter(enc, (long)format);
                    ImageCodecInfo info = GetEncoderInfo("image/tiff");
                    ...
                    newImage.Save(stream, info, ep);
                    splitedFiles.Add(stream);
        ...
        /// <param name="outFile">target TIFF file to be produced</param>
        /// <param name="compressEncoder">compression codec enum</param>
        public static bool JoinTiffImagesStream(ArrayList imageFiles, string outFile, EncoderValue compressEncoder)
        {
            try
            {
                //If only one page in the collection, copy it directly to the target file.
                ...
                //ArrayList splittedImageFiles = SplitTiffImage3(imageFiles, compressEncoder)

                foreach (Stream strImageFile in imageFiles)
                {
                    if (frame == 0)
                    {
                        pages = (Bitmap)Image.FromStream(strImageFile);
                    ...
                    else
                    {
                        ...
                        Bitmap bm = (Bitmap)Image.FromStream(strImageFile);
                        pages.SaveAdd(bm, ep);
                        bm.Dispose();
                    }
        ...
        /// <summary>
        /// ch**************para leer todo el contenido de un stream y devolver un arreglo de bytes[]*****************ch
        /// </summary>
        /// <param name="source"></param>
        /// <returns></returns>
        public static byte[] getAllBytes(Stream source)
        {
        ...
        /// <summary>
        /// ch**************para leer todo el contenido de un ArrayList de imagenes y unirlos en un archivo .tiff en memoria y devolverlo en un arreglo de bytes[]*****************ch
        /// </summary>
        /// <param name="imageFiles"></param>
        /// <param name="compressEncoder"></param>
        /// <returns></returns>
        public static byte[] JoinTiffImagesStreamMS(ArrayList imageFiles, EncoderValue compressEncoder)
        {
           Stream outputStream = new MemoryStream();
            try
            {
                ...
                foreach (Stream strImageFile in imageFiles)
                {
                    if (frame == 0)
                    {
                        pages = (Bitmap)Image.FromStream(strImageFile);
                        //save the first frame
                        pages.Save(outputStream, info, ep);
                    }
                    else
                    {
                        //save the intermediate frames
            ...
            return getAllBytes(outputStream);
        }
